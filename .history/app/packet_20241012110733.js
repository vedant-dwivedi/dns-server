class Packet {
	constructor(
		...
		this.header = bitID + bitQR + bitOPCODE + bitAA + bitTC + bitRD + bitRA + bitZ + bitRCODE + bitQDCOUNT + bitANCOUNT + bitNSCOUNT + bitARCOUNT;
		this.header = Packet.bitsToBuffer(bitID + bitQR + bitOPCODE + bitAA + bitTC + bitRD + bitRA + bitZ + bitRCODE + bitQDCOUNT + bitANCOUNT + bitNSCOUNT + bitARCOUNT);
		this.question = '';
	}
	...
	static intToBigEndianBits(value, numBits) {
		...
		for (let i = numBits - 1; i >= 0; i--) {
			// Extract each bit using a bitwise AND between the value and a shifted bit mask
	...
	addQuestion() {
		...
		const type = Packet.bitsToBuffer(Packet.intToBigEndianBits(1, 16));
		const classField = Packet.bitsToBuffer(Packet.intToBigEndianBits(1, 16));
		this.question = Buffer.concat([name, type, classField], name.length + 2 + 2);
		return this;
	}
	getResponse() {
		return Packet.bitsToBuffer(this.header);
		const header = this.header;
		const question = this.question;
		const response = Buffer.concat([header, question], 12 + question.length);
		console.log(response.length, 'responseBit.length');
		console.log(response.toString(), '>>>> response string');
		return response;
	}
}
module.exports =
